...
# Get case
def get_case(caseId):
    response = requests.get(f"{HIVE_URL}/api/v1/{caseId}", headers=headers, verify=False)
...
# Get Task
def get_task(taskId):
    response = requests.get(f"{HIVE_URL}/api/v1/task/{taskId}", headers=headers, verify=False)
